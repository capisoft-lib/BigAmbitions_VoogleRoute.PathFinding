...
- `python -m unittest discover -s tools -p test_repair_deadend_turns.py`: **7 passed**. Includes byte-exact regeneration from the recorded baseline rows, idempotence, all sampled curves behind the terminal plane, duplicate rejection and no write on coordinate drift.
